  ...
  <div id="app" >
    ...
    <div class="login_top">
      <ul class="login_nav">
        ...
        <li v-if="!this.$store.state.user" @click="changeLoginway('login')"><router-link to="/login"  class="login">Login</router-link></li>
        <li v-if="!this.$store.state.user" @click="changeLoginway('register')" ><router-link to="/login"class="register">Register</router-link></li>
        <li v-else ><router-link to="/shop">{{this.$store.state.user}}</router-link></li>
      </ul>
    </div>
    <div class="nav">
      <ul class="left_nav">
      ...
      <h1 class="theme">
        <router-link to="/home">webGIS实习</router-link>
      </h1>
    </div>
    </header>
    <section>
    ...
    <footer>
      <strong><a href="https://github.com/RachelHui12/webgis-.git" target="_blank">author:熊淑萍</a></strong>
      <h3>学号：2017302590041</h3>
    </footer>
...
<style>
      ...
      .login_top{background: #333333;height: 30px;}
      ...
      footer{height: 72px;background: #333333;width: 100%;min-width: 1150px;padding-top: 8px;position: relative;color:#a9b8ca;}
      .bg{background: #00BC9B;}
      footer strong {position: absolute;left: 600px;top:15.5px;}
      footer strong a{text-decoration: underline;color:white;}
      footer h3{position: absolute;left: 600px;top:38.5px;color:white;}
